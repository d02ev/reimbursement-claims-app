import { Component, OnInit } from '@angular/core';
import { ClaimService, ErrorService } from 'src/app/services';

@Component({
  selector: 'app-declined-claims',
  templateUrl: './declined-claims.component.html',
  styleUrls: ['./declined-claims.component.css'],
})
export class DeclinedClaimsComponent implements OnInit {
  constructor(
    private readonly _claimService: ClaimService,
    private readonly _errorService: ErrorService,
  ) {}

  // public attributes
  public isError = false;
  public errorResponse = '';
  public isInfo = false;
  public infoResponse = 'No Declined Claims Found!';
  public declinedClaims: any = [];

  // public methods
  ngOnInit(): void {
    this._claimService.getAllDeclinedClaims().subscribe({
      next: (response: any) => {
        if (response.length === 0) {
          this.isInfo = true;
        }

        this.declinedClaims = response;
      },
      error: () => {
        this.isError = true;
        this.errorResponse = this._errorService.errorMessage;
        setTimeout(() => {
          this.isError = false;
        }, 5000);
      },
      complete: () => {
        setTimeout(() => {
          this.isInfo = false;
        }, 4500);
      },
    });
  }
}
